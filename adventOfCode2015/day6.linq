...
void Main()
{
	// Routine to solve day 6 of the AdventOfCode 2015
	// http://adventofcode.com/day/6
	var raw  = GetRawInstructions();
	var instructions = GetProcessedInstructions(raw);
	//instructions.Take(10).Dump();

	bool[] lights = new bool[1000 * 1000];
	for (int fillindex = 0; fillindex < 1000 * 1000; fillindex++)
	{
		lights[fillindex] = false;
	}
	...
	{
		for (int xindex = instruction.BottomLeft.X; xindex <= instruction.TopRight.X; xindex++)
		{
			for (int yindex = instruction.BottomLeft.Y; yindex <= instruction.TopRight.Y; yindex++)
			{
				int index = xindex * 1000 + yindex;
				switch (instruction.Action)
				{
					...
					case LightAction.TurnOff:
						lights[index] = false;
						lights2[index] -= 1;
						if (lights2[index] < 0) lights2[index] = 0;
						break;
	...
	lights.Where(v => v == true).Count().Dump("Number of lights lit");
	lights2.Sum(v => v).Dump("Brightness of lights");
}

public IEnumerable<Instruction> GetProcessedInstructions(IEnumerable<string> rawInstructions)
{
	Regex re = new Regex(@"(?<action>((toggle)|(turn on)|(turn off))) (?<x1>\d*),(?<y1>\d*) through (?<x2>\d*),(?<y2>\d*)");
	//Regex re = new Regex(@"(((turn on)|(turn off)|(toggle))[ ]){1}(?<x1>\d*),(?<y1>\d*) through (?<x2>\d*),(?<y2>\d*)");
	List<Instruction> result = new List<Instruction>();
	foreach (string rawInstruction in rawInstructions)
	{
		var match = re.Match(rawInstruction);
		if (!match.Success)
		{
			throw new Exception("Match error in instruction: " + rawInstruction);
		}
		string actionstr = match.Groups["action"].Value;

		Instruction instruction = new Instruction
		{
			Action = actionstr == "turn on" ? LightAction.TurnOn : actionstr == "turn off" ? LightAction.TurnOff : LightAction.Toggle,
			BottomLeft = new Point(int.Parse(match.Groups["x1"].Value), int.Parse(match.Groups["y1"].Value)),
			TopRight = new Point(int.Parse(match.Groups["x2"].Value), int.Parse(match.Groups["y2"].Value))
		};
...
public IEnumerable<string> GetRawInstructions()
{
